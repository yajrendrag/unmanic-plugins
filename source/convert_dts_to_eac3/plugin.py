...
def s2_encode(probe_streams, abspath):
    try:
        dts_streams_list = [i for i in range(0, len(probe_streams)) if "codec_type" in probe_streams[i] and probe_streams[i]["codec_type"] == 'audio' and int(probe_streams[i]["channels"]) >= 6 and probe_streams[i]["codec_name"] in ["dts", "truehd"]]
    ...
    if stream_to_encode:
        ...
        for i in range(len(all_astreams)):
            if all_astreams[i] in stream_to_encode:
                logger.info("audio stream '{}' is encoded with dts and will be re-encoded as eac3 replacing original dts audio stream".format(i))
    else:
        logger.info("do not add file '{}' to task list - no 6 (or more) channel, dts audio streams found".format(abspath))
