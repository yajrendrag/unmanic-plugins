...
def stream_has_ffprobe_data(path, probe_streams, probe_field, probe_value):
    ...
    streams_to_remove = [probe_streams[i]['index'] for i in range(0, len(probe_streams)) for j in range(0, len(probe_field)) if (probe_field[j] in probe_streams[i] and probe_value[j] in probe_streams[i][probe_field[j]]) or (probe_field[j] in probe_streams[i]["tags"] and probe_value[j] in probe_streams[i]["tags"][probe_field[j]])]
